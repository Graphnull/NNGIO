...
napi_value init(napi_env env, napi_value exports)
{
	...
	status = napi_create_function(env, nullptr, 0, initialize, nullptr, &fn); //создаем функцию для экспорта
	if (status != napi_ok)
		return nullptr;

	status = napi_set_named_property(env, exports, "init", fn); //назначаем функцией имя
	if (status != napi_ok)
		return nullptr;
